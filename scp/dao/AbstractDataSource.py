...
class AbstractDataSource(metaclass=ABCMeta):
    ...
    def _get_connection_config(self):
        ...
        취득해 오는 정보는 데이터 소스에 따라 다름
            DbDataSource -> Database 접속 관련 설정 값들
                : IP, Port, SID, User, Pwd 등
            FileDataSource -> FileSystem 접속 관련 설정 값들
                : 현재 패키지가 설치된 로컬 환경 내에 있는 파일 경로
        :return: dict ?
        """
        pass
    ...
    def _release_connection(self, connection: object):
        ...
        해제되는 접속은 데이터 소스에 따라 다름
            DbDataSource - cx_Oracle.SessionPool
                : cx_Oracle.SessionPool.release( cx_Oracle.Connection )
            FileDataSource - File IO 를 위한 객체
                : .close()
        :param connection: Connection 인스턴스
        :return: void
        """
        pass
