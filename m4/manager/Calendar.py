...
class Calendar(object):
    ...
    def __init__(self):
        ...
        self._horizonSec: int = 0                           # 총 캘린더 길이: int 초 단위
        self._timestepSec: int = 0                          # 시간 간격: int 초 단위
        self._timestepDelta: datetime.timedelta = None      # 시간 간격: timedelta 초 단위
        self._fullCalendar: list = []                       # 전체: [dict: {datetime, 가동여부}] / build_full_calendar()

    def setup_calendar(self, start_date: object, timestep_length: int, timestep_uom: str,
        ...
        # 시작 일자 세팅 전 검사 및 변환 단계
        start_date = self._check_date_info(start_date)

        # 시작 일자 정보 세팅
        self.set_start_date(start_date=start_date)

        # horizon 길이 세팅 : 사용자 지정 UOM 단위 ()
        self.set_horizon(horizon_length=horizon_length, uom=horizon_uom)

        # timestep 길이 세팅 : 사용자 지정 UOM 단위 ()
        self.set_timestep(timestep_length=timestep_length, uom=timestep_uom)
    ...
    def build_full_calendar(self):
        ...
        for sec in range(0, self._horizonSec + self._timestepSec, self._timestepSec):
    ...
    def set_horizon(self, horizon_length: int, uom: str):
        ...
        self._horizonSec = int(horizon_timedelta.total_seconds())
    ...
    def set_timestep(self, timestep_length: int, uom: str):
        ...
        self._timestepSec = int(timestep_timedelta.total_seconds())

    def get_timestep(self):
        """
        해당 캘린더에 세팅된 _timestep(int: 초 단위) 값을 반환합니다.
        :return: 시간 구간 길이 (int: 초 단위)
        """
    ...
    def _build_timestep_delta(self):
        ...
        :return: void
        """
        self._timestepDelta = datetime.timedelta(seconds=self._timestepSec)
