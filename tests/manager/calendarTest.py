...
class CalendarTestCase(unittest.TestCase):
    ...
    def test_uom_timedelta(self):
        ...
        ten_hours_delta: datetime.timedelta = self.calendar._derive_timedelta(time_length=10, uom="hours")
        print(f"{10} {'hours'} = {type(ten_hours_delta)} : {ten_hours_delta}")

        one_week_delta: datetime.timedelta = self.calendar._derive_timedelta(time_length=1, uom="weeks")
        print(f"{1} {'weeks'} = {type(one_week_delta)} : {one_week_delta}")

        one_microsec_delta: datetime.timedelta = self.calendar._derive_timedelta(time_length=1, uom="microseconds")
    ...
    def test_setup_with_datetime(self):
        ...
        # start_date 가 datetime 인스턴스로 넘어올 경우를 상정
        start_date: datetime.datetime = datetime.datetime(year=2020, month=3, day=23)

        # Test 정보 출력
        print(f"\tSTART_DATE : {start_date}")
        print(f"\tTIME_STEP : {1} {'hours'}")
        print(f"\tHORIZON : {1} {'weeks'}")

        # start_date 파라메터가 datetime 인스턴스로 넘어올 경우 별도의 처리 없이 진행되도록 설계
        self.calendar.setup_calendar(start_date=start_date, timestep_length=1, timestep_uom="days",
                                     horizon_length=15, horizon_uom="days")

        # 실제 세팅된 캘린더 목록을 Loop 돌면서 확인
        full_calendar: list = self.calendar.get_full_calendar()
    ...
    def test_setup_with_str(self):
        ...
        # start_date 는 위에서 정의된 바와 같이 문자열이지만,
        # 내부에서 알아서 datetime 변환 처리
        self.calendar.setup_calendar(start_date=start_date, timestep_length=1, timestep_uom="hours",
                                     horizon_length=1, horizon_uom="weeks")
    ...
    def test_setup_with_str_microsec_step(self):
        ...
        start_date: str = "2020-03-23 00:00:00"
        print(f"\tSTART_DATE : {start_date}")
        print(f"\tTIME_STEP : {1} {'microseconds'}")
        print(f"\tHORIZON : {1} {'hours'}")
        ...
        # 내부에서 알아서 datetime 변환 처리
        try:
            self.calendar.setup_calendar(start_date=start_date, timestep_length=1, timestep_uom="microseconds",
                                         horizon_length=1, horizon_uom="hours")
...
if __name__ == '__main__':
    tester: CalendarTestCase = CalendarTestCase()
