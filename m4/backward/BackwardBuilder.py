...
from m4.dao.InventoryDAO import InventoryDAO
...
class BackwardBuilder:
    ...
    def _get_dao_data(cls, simulation_dict: dict, session: AbstractSession):
        ...
        # IntenvotryDAO로 부터 정보 받아오기
        inventory_dao: InventoryDAO = InventoryDAO.instance()
        inventory_master = inventory_dao.map(inventory_dao.select(session=session))
        ...
        bor_master = bor_dao.map(bor_dao.select(session=session))

        return work_order_master, route_master, inventory_master, inventory_item_master, wip_master, bor_master

    @classmethod
    def _create_setup_time_dict(cls, bor_list: list):
        setup_time_dict = {}
        for bor in bor_list:
            key, value = bor['PROC_ID'], bor['PRE_PROC_SETUP_TM']
            if key not in setup_time_dict:
                setup_time_dict.update({key: [value]})
            else:
                temp_list = setup_time_dict[key]
                temp_list.append(value)
                setup_time_dict.update({key: temp_list})

        # 각 Process별 Resource Time의 평균값으로 변환
        for key, value in setup_time_dict.items():
            avg = mean(value)
            setup_time_dict.update({key: avg})
    ...
    def _create_proc_time_dict(cls, bor_list: list):
        proc_time_dict = {}
        for bor in bor_list:
            key, value = bor['PROC_ID'], bor['PROC_TM']
            if key not in proc_time_dict:
                proc_time_dict.update({key: [value]})
            else:
                temp_list = proc_time_dict[key]
                temp_list.append(value)
                proc_time_dict.update({key: temp_list})

        # 각 Process별 Resource Time의 평균값으로 변환
        for key, value in proc_time_dict.items():
            avg = mean(value)
            proc_time_dict.update({key: avg})
