    ...
    def select_route_inventory(self, session: AbstractSession, **params):
        ...
        return session.select(
            """
                select *
                from FS_INV
                where INV_ID IN (select CURR_LOC_ID from FS_ROUTE where sim_id = :simulation_id and CURR_LOC_TYP = 'INV')
                   OR INV_ID IN (select NEXT_LOC_ID from FS_ROUTE where sim_id = :simulation_id and NEXT_LOC_TYP = 'INV')
            """, params)
